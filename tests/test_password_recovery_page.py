...
class TestPasswordRecovery:
    ...
    def test_enter_mail_click_restore_button_go_to_restore_page(self, driver):
        ...
        login_page = LoginPageHelper(driver=driver, url=driver.current_url)
        login_page.click_on_the_text_recover_password()
        recover_password_page = ForgotPasswordPageHelper(driver=driver, url=driver.current_url)
        ...
        assert recover_password_page.find_element(
            ForgotPasswordPageLocators.FIELD_ENTER_THE_CODE_FROM_THE_LETTER), "Не найдено поле 'Введите код из письма'"
    ...
    def test_click_on_the_eye_makes_field_active(self, driver):
        ...
        assert recover_password_page.find_element(
            locator=ForgotPasswordPageLocators.FIELD_PASSWORD_IS_ACTIVE)
